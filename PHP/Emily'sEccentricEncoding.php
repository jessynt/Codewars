...
function decode($string)
{
	...
	// $map = array_combine($arr1, array_values($arr2));
	// $result = '';
	// foreach ($chars as $key => $char) {
	// 	$result .= array_key_exists($char, $map)?$map[$char]:$char;
	// }
 //    return $result;
	$charset = implode('', range('a', 'z'));

	return strtr($string, $charset, strrev($charset));

}
dd(decode("I dlfow orpv gl rmgviervd lm Mlmwzb nlimrmt!"));
